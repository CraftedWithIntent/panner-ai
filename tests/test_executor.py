...
def mock_suite_config():
    """Create a test SuiteConfig fixture."""
    test_cases = [
        TestCaseSpec(
            name="test_1",
...
class TestAgentResponse:
    """Test AgentResponse immutability."""

    def test_agent_response_frozen(self):
        """AgentResponse is frozen."""
...
class TestBaselineLoading:
    ...
    def test_load_baseline_exists(self):
        """Load baseline when file exists."""
    ...
    def test_load_baseline_missing(self):
        """Load baseline when file missing returns None."""
        executor = TestExecutor(
            config=MagicMock(),
    ...
    def test_load_baseline_invalid_json(self):
        """Load baseline with invalid JSON returns None."""
        with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False) as f:
...
class TestExecuteSingleTest:
    ...
    async def test_execute_single_test_success(self, mock_suite_config):
        """Execute test with successful response."""
        ...
        test_name, response = await executor._execute_single_test(
            mock_client, test_spec
        )
    ...
    @pytest.mark.asyncio
    async def test_execute_single_test_text_body(self, mock_suite_config):
        """Execute test and parse text body."""
        executor = TestExecutor(config=mock_suite_config)
        test_spec = mock_suite_config.test_cases[0]
